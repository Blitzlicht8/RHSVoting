...
// Bump when the DDL/seed block below changes so cold instances re-run it once.
const SCHEMA_VERSION = '2.0.0'

async function _init(): Promise<void> {
  // Cold-start killer fix: every serverless cold instance used to run 27 CREATE
  // TABLE + ~55 ALTER + seeds + 2 bcrypt.hash on the request hot path. Gate the
  // whole thing behind a cheap one-row read: if the schema is already current,
  // return immediately. Wrapped in try/catch because on a truly fresh DB the
  // settings table doesn't exist yet — fall through to the full init then.
  try {
    const r = await db.execute({
      sql: `SELECT value FROM settings WHERE key = 'schema_version'`,
      args: [],
    })
    if ((r.rows[0]?.value as string | undefined) === SCHEMA_VERSION) return
  } catch {
    /* settings table missing (fresh DB) → run full init below */
  }
  ...
  // Stamp schema version LAST so a partial init (crash mid-DDL) re-runs next
  // time rather than being skipped. Subsequent cold starts short-circuit above.
  await db.execute({
    sql: `INSERT OR REPLACE INTO settings (key, value) VALUES ('schema_version', ?)`,
    args: [SCHEMA_VERSION],
  })
