...
**2. Reverification is now DERIVED from required-group completeness, not an admin action.** New `getMissingRequiredStructures(userId)` in `src/lib/groups.ts` = active `is_required` structures the user has no value for. An admin *changing* a user's group never forces reverify (PATCH deliberately does not touch `id_verified`); a user only becomes "treated as unverified" when a required structure is left blank (e.g. its value was force-deleted). Force-delete paths (`groups/values/[vid]` DELETE, `groups/[id]` DELETE) no longer blanket-set `id_verified=0`/`needs_academic_update=1` — value delete just clears the `user_group_values` row (derived check flags them only if that structure is required); structure delete removes the requirement itself so no one is flagged. Vote route (`elections/[id]/vote`) now 403s if `getMissingRequiredStructures` non-empty (after the `id_verified` gate). `GET /api/auth/me` returns `missing_required_groups: string[]`.

**3. "Update Info" banner now actually reverifies.** `src/components/Layout.tsx` banner keys off derived `missing_required_groups` (was the stale `needs_academic_update` flag), lists the missing structure name(s), and links to **`/verify-id`** (was the dead `/profile` link). `verify-id/page.tsx` `deriveUiState` forces the `upload` (re-select groups + resubmit doc) state when `missing_required_groups` is non-empty, overriding the `verified` short-circuit, so an already-id-verified user with a blanked required group is sent back through group selection + document submission. Types added to `AuthProvider.User` and verify-id's local `User`.

`tsc --noEmit` clean, `npm run build` green. Live auth-gated visual pass on deploy still wanted (remote-DB). See Session 16 note below for the prior editor redesign.

Prev: `2.4.0` — MINOR: **Group Structure Editor Redesign (Session 16, `feat/group-structure-ux`)**. Replaced the janky flat sibling-tab editor on `src/app/admin/academic/page.tsx` with a hierarchical drill-down — no DB/API change (still `group_structures.parent_structure_id` / `group_values.parent_value_id` over `GET /api/admin/groups?tree=1` + `/api/admin/groups/[id]/values?parentValueId=`). **Root selector** now lists only root structures (`parent_structure_id == null`), tagged `▾ leveled` when a child chain exists. **Drill-down**: click a parent value (whole left area is the affordance, shows the child structure name + chevron) to descend into its child values; a **breadcrumb** (`Strand › STEM › Section`) shows location and each crumb is clickable to jump back up — replaces the old "pick structure tab, then pick parent-value in a separate dropdown" two-step. Values render as full-width rows (name + user_count badge + inactive tag + hover rename/toggle/delete). **All CRUD preserved**: add (parent_value_id = current drill parent), rename (inline), activate/deactivate, delete incl. the 409 force-delete-with-users flow + both confirm modals. **Verifier panel** kept (search / list / assign / remove) — restyled to Session-15 light theme. **Real skeletons** (`animate-pulse`) replace the "Loading structures…/Loading…" text that read as broken. `derives currentStructureId/currentParentValueId/childStructure` from `rootStructureId` + a `drillPath` stack; drill path auto-resets if the tree reloads and no longer resolves. Everything reads structure/value names live (no hardcoding). `tsc --noEmit` clean, `npm run build` green. Live auth-gated visual pass on deploy still wanted (remote-DB, not headless-runnable). See Session 16 below.

Prev: `2.3.0` — MINOR: **UI/UX & Contrast Overhaul (Session 15, `fix/ui-contrast`)**. Fixed the confirmed invisible-text bug: `src/app/admin/academic/page.tsx` (Group Structure) was the **only** admin page built for a dark theme (`text-white`, `bg-gray-900`, `text-gray-300`) while the admin shell content area is light (`bg-gray-50` + white `Navbar`). Its `text-white` "Group Structure" heading + every card/input/pill/modal landed dark-on-light or white-on-white and vanished. Converted the entire page to the shared light admin theme every other admin page uses (`bg-white` cards + `border-gray-200 shadow-sm`, `text-gray-900` headings, `text-gray-500/600/700` body, `bg-white`/`border-gray-200`/`text-gray-900` inputs with `focus:ring-[#84050C]/20`, `bg-gray-100 text-gray-700` chips, light modals). All CRUD + the Verifier assignment panel keep their function — restyle only. Also dropped the page's double `p-6` wrapper (AdminLayout already pads `p-4 md:p-6 lg:p-8`). **Structural fix, not a spot-patch** — the mismatch can't recur because the page no longer assumes a dark theme. App-wide sweep confirmed no other dark-themed page/card leftovers (`bg-gray-900/800` only appears in the intended dark sidebar/nav + modal `bg-black/60` overlays) and no orphan white-on-light text; remaining `text-gray-200/300` are decorative icons, empty-state graphics, `—` dashes, disabled/unchecked labels, and the 404 numeral — none are readable body text. `tsc --noEmit` clean, `npm run build` green. See Session 15 below.
